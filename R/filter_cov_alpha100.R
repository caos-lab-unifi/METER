...
#' This file must be produced using the `bismark_methylation_extractor` function with the -`-comprehensive` option, as strand-specific methylation information is not required.
#' The file should contain five columns reporting in the following order: sequencing read ID, methylation status, chromosomal position, genomic position, and methylation call of the
#' CpG covered by the sequencing read. For more details, consult the Bismark manual: https://felixkrueger.github.io/Bismark/bismark/.
...
#'
#' @return A "coverage" file generated using the `bismark2bedGraph` function from Bismark, created considering only reads with
#' an alpha value of 0 or 1 (fully unmethylated or fully methylated) and covering a number of CpG sites greater than or equal to `min_sites`.
#' @export
filter_cov_alpha100 <- function(path_bismark2bedGraph, path_cpg_file, path_read_table, path_out = NULL, min_sites = 6, remove_cpg=FALSE){
  ...
  assertthat::assert_that(all(c("seq_id", "chr", "min_cpg_pos", "max_cpg_pos", 
                                "n_meth", "n_unmeth", "n_sites", "meth_perc") 
                              %in% colnames(read_table)),
