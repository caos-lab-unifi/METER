...
#'
#' @param path_cpg_file The absolute path to the 'CpG_context' file generated by Bismark for the analyzed sample.
#' This file must be produced using the `bismark_methylation_extractor` function with the -`-comprehensive` option,
#' as strand-specific methylation information is not required. The file should contain five columns reporting in the following order:
#' sequencing read ID, methylation status, chromosomal position, genomic position, and methylation call of the CpG covered by the sequencing read.
#' For more details, consult the Bismark manual: https://felixkrueger.github.io/Bismark/bismark/.
#' @param dmr_table A DataFrame specifying the chromosomal and genomic positions of selected DMR, with at least 5 required columns:
#' `dmr_id` (unique identifier for each DMR), `chr` (chromosome, e.g., 'chr1', 'chr2'), `start` (starting genomic position), `end` (ending genomic position),
#' and `type` (methylation status, either 'hypo' for hypomethylated or 'hyper' for hypermethylated DMR).
#'
#' @return A DataFrame (Read Table) summarizing each sequencing read included in the 'CpG_context' file of a sample, including the columns:
#' `seq_id` (the sequence read identifier), `min_cpg_pos` (position of the first CpG within the read), `max_cpg_pos` (position of the last CpG within the read),
#' `n_meth` (number of methylated CpGs), `n_unmeth` (number of unmethylated CpGs), `n_sites` (total CpGs covered), and `meth_perc` (methylation percentage, i.e. n_meth / n_sites).
#' If a DMR Table is provided, it also includes `dmr_id` (unique identifier of the overlapping DMR) and `dmr_type` ('hypo' for hypomethylated or 'hyper' for hypermethylated DMR),
#' where overlapping means the read is fully contained within the DMR. 
#' @export
#'
#' @import data.table
create_read_table <- function(path_cpg_file, dmr_table=NULL){
  ...
  if (is.null(dmr_table)) {
    ...
    assertthat::assert_that(all(c("dmr_id", "chr", "start", "end", "type") %in% colnames(dmr_table)),
    ...
    if (nrow(overlap) == 0) {
      warning("No common CpG between sequencing reads and DMR")
      cpg_file$dmr_id <- NA
      cpg_file$dmr_type <- NA
      return(cpg_file)
    }
