...
-   **Input data and parameters**:
    ...
    -   dmr_table (optional, required for METER-detect module): a
        ...
        selected DMR, containing at least 4 columns: “dmr_id” (unique
...
-   **Output**:
    -   Read Table: a DataFrame containing summarized information for
        ...
        and the total number of CpGs covered (“n_sites”). Additionally,
        if a DMR Table is provided, it specifies, for each read, the
        overlapping DMR (“dmr_id”) and the type of the overlapping DMR
        (“hypo” or “hyper”), where “overlapping” means the read is
        entirely contained within an DMR.
...
-   **Output**:
    -   “Coverage file” generated by *bismark2bedGraph* function from
        ...
        of 100% and covering a number of CpG sites ≥ min_sites.
...
-   **Output**:
    -   Beta Table: a DataFrame containing the beta values (as
        proportions) of DMS across samples, with samples as column names
        and DMS as row names. Each DMS is reported with its unique
        “dms_id”, matching the identifiers provided in the DMS table.
        Beta values are computed considering only reads with alpha value
        of 100% and covering a minimum number of CpGs, as explained
        above.
...
-   **Input data and parameters**:
    ...
    -   beta_table: a DataFrame as the one created in the previous step,
        ...
        should be computed considering only reads with alpha value of
        100% and covering a minimum number of CpGs, as explained above.
-   **Output**:
...
-   **Input data and parameters**:
    -   dmr_table: a DataFrame reporting the chromosomal and genomic
        ...
        position), “end” (ending genomic position), and “type” (“hypo”
        for hypomethylated DMR or “hyper” for hypermethylated DMR). For
        breast cancer samples, the METER package provides a DataFrame
...
-   **Input data and parameters**:
    -   dmr_beta: a DataFrame (as obtained through the
        ...
        the beta by DMR values (as proportions) of selected
        subtype-specific DMR across samples to be inspected, with
        samples as column names and DMR as row names. Each DMR must be
        reported with a unique “dmr_id”.
...
-   **Output**:
    -   a DataFrame containing for each sample (row names) the estimated
        ...
        An additional column labeled “pred_subtype”, alongsides columns
